...
export default function PartnershipConfiguratorSlide({ contentData, dark }: SlideComponentProps) {
  return (
      ...
      <div className="max-w-5xl w-full mx-auto">
        ...
        >
          <h2 className={`text-2xl md:text-4xl font-bold mb-3 transition-colors duration-500 ${dark ? 'text-white' : 'text-gray-900'}`}>
            {(contentData.title as string) || 'Our partnership product'}
          </h2>
          <p className={`text-sm md:text-base transition-colors duration-500 ${dark ? 'text-white/40' : 'text-gray-400'}`}>
            {(contentData.subtitle as string) || 'Configure your connection infrastructure engagement'}
          </p>
        </motion.div>
