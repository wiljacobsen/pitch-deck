...
const DATA: BarData[] = [
  ...
  { year: '2025', segments: [
    { value: 7, color: COLORS.nem }, { value: 2.5, color: COLORS.swis },
    { value: 2, color: COLORS.pilbara }, { value: 2, color: COLORS.dcLoad }, { value: 2, color: COLORS.dcGen },
  ]},
  { year: '2026', segments: [
    { value: 12, color: COLORS.nem }, { value: 4, color: COLORS.swis },
    { value: 3, color: COLORS.pilbara }, { value: 5, color: COLORS.dcLoad }, { value: 4, color: COLORS.dcGen },
  ]},
  { year: '2027', segments: [
    { value: 14, color: COLORS.nem }, { value: 5, color: COLORS.swis },
    { value: 4, color: COLORS.pilbara }, { value: 6, color: COLORS.dcLoad }, { value: 6, color: COLORS.dcGen },
  ]},
  { year: '2028', segments: [
    { value: 18, color: COLORS.nem }, { value: 7, color: COLORS.swis },
    { value: 5.5, color: COLORS.pilbara }, { value: 8, color: COLORS.dcLoad }, { value: 8, color: COLORS.dcGen },
  ]},
  { year: '2029', segments: [
    { value: 20, color: COLORS.nem }, { value: 8, color: COLORS.swis },
    { value: 7, color: COLORS.pilbara }, { value: 11, color: COLORS.dcLoad }, { value: 12, color: COLORS.dcGen },
  ]},
]
...
export default function WhyWeExistSlide({ contentData, dark }: SlideComponentProps) {
  ...
  const subtitle = (contentData.subtitle as string) || 'Future project connections hinge on execution capability and capacity. Symphony provides certainty of delivery for our partners.'
  const bulletPoints = (contentData.bulletPoints as string[]) || DEFAULT_BULLETS
      ...
      <div className="max-w-6xl w-full mx-auto">
        ...
        >
          ...
          <p className={`text-sm md:text-base transition-colors duration-500 max-w-3xl ${dark ? 'text-white/60' : 'text-gray-500'}`}>
            {subtitle.split('Symphony provides').map((part, i) =>
              i === 0 ? <span key={i}>{part}</span> : <span key={i}><span className="text-accent font-semibold">Symphony provides</span>{part}</span>
            )}
          </p>
        </motion.div>
        ...
        <div className="relative w-full">
          <div className="flex">
            <div className="relative shrink-0 w-8 mr-1" style={{ height: CHART_HEIGHT }}>
              {Y_TICKS.map((val) => (
                <span key={val} className={`absolute right-0 -translate-y-1/2 text-[10px] md:text-xs ${textColor}`} style={{ bottom: `${(val / MAX_VALUE) * 100}%` }}>
                  {val}
                </span>
              ))}
            </div>
            <div className="flex-1 relative" style={{ height: CHART_HEIGHT }}>
              {Y_TICKS.map((val) => (
                <div key={val} className="absolute left-0 right-0 h-px" style={{ bottom: `${(val / MAX_VALUE) * 100}%`, backgroundColor: gridColor }} />
              ))}
                      ...
                      >
                        <div className="w-full h-full flex flex-col-reverse">
                          {bar.segments.map((seg, segIdx) => (
                            <div key={segIdx} style={{ height: `${(seg.value / total) * 100}%`, backgroundColor: seg.color, minHeight: 1 }} />
                          ))}
                        </div>
                      </motion.div>
                    </div>
              ...
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: dark ? '0 4px 24px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.04)' : '0 4px 24px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.8)',
                }}
              ...
              >
                <ul className="space-y-2.5 mb-4">
                  {bulletPoints.map((point, i) => (
                    <motion.li key={i} className={`flex items-start gap-2.5 text-xs md:text-sm leading-relaxed ${dark ? 'text-white/85' : 'text-gray-700'}`}
                      initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.35 + i * 0.1, duration: 0.4 }}
                    >
                ...
                <motion.div className="flex flex-wrap gap-x-4 gap-y-1.5" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.6, duration: 0.4 }}>
                  {LEGEND.map((item) => (
            ...
            <div className="relative shrink-0 w-40 ml-3" style={{ height: CHART_HEIGHT }}>
              {ANNOTATIONS.map((ann, i) => (
                <motion.div key={ann.label} className="absolute flex items-center gap-2"
                  style={{ bottom: `${(ann.valueMid / MAX_VALUE) * 100}%`, transform: 'translateY(50%)' }}
                  initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.8 + i * 0.1, duration: 0.4 }}
                >
                  ...
                  <span className="text-[9px] md:text-[11px] font-semibold px-2 py-0.5 rounded whitespace-nowrap"
          ...
          <div className="flex ml-9 mr-[172px]">
            <div className="flex-1 flex gap-[3px] md:gap-1.5 px-1">
              {DATA.map((bar) => (
                <div key={bar.year} className={`flex-1 text-center text-[9px] md:text-[11px] pt-1 ${textColor}`}>{bar.year}</div>
              ))}
            </div>
          </div>
          <div className={`absolute left-0 text-[10px] md:text-xs font-medium ${textColor} whitespace-nowrap`}
            style={{ top: '50%', transform: 'rotate(-90deg) translateX(-50%)', transformOrigin: '0 0' }}
          >
            Annual HV Install Capacity (GW)
          </div>
          <motion.p className={`mt-2 text-[8px] md:text-[10px] ${textColor} ml-9 italic`}
            initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 1.2, duration: 0.5 }}
          >
            {footerNote}
          </motion.p>
        </div>
